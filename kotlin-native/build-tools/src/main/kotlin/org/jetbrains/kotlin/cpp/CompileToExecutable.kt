...
private abstract class CompileToExecutableJob : WorkAction<CompileToExecutableJob.Parameters> {
    ...
    private fun llvmLink() {
        with(parameters) {
            ...
            execOperations.exec {
                executable = "${llvmDir.asFile.get()}/bin/llvm-link"
    ...
    private fun compile() {
        with(parameters) {
            val platformManager = PlatformManager(buildDistribution(konanHome.asFile.get().absolutePath), experimentalDistribution.get())
            val execClang = ExecClang.create(objects, platformManager, llvmDir.asFile.get())
...
abstract class CompileToExecutable : DefaultTask() {
    ...
    fun compile() {
        ...
        workQueue.submit(CompileToExecutableJob::class.java) {
            ...
            llvmDir.set(project.file(project.property("llvmDir")!!))
